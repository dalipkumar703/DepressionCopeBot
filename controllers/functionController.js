...
//set Get setGetStarted
exports.setGetStarted=function(){
  var messageData={
    "get_started":{
  ...
  request({
    url: process.env.FACEBOOK_GET_STARTED,
    qs: {
    ...
    if (!error && response.statusCode == 200) {
      ...
      console.log("Get started is set");
    } else {
...
//call x payload only
exports.callSendWithXPayloadOnly = function(recipient, payload, title, text) {
  var element = [];
  for (var i = 0; i < payload.length; i++) {
    var obj_postback = {
      "type": "postback",
